\begin{center}
%{\huge (Septiembre, 2025)}
\end{center}
\begin{table*}[ht]
		\centering
		\makebox[\textwidth]{
			\begin{tabular}{|l|l|}
				\cline{1-2}
				\multicolumn{2}{|c|}{}	\\
				\multicolumn{2}{|c|}{\textbf{UNIVERSIDAD DE LEÓN}}	\\
				\multicolumn{2}{|c|}{\textbf{Departamento de Ingenierías Mecánica, Informática y Aeroespacial}}	\\ 
				\multicolumn{2}{|c|}{\textbf{MÁSTER UNIVERSITARIO EN ROBÓTICA E INTELIGENCIA ARTIFICIAL}}	\\
				\multicolumn{2}{|c|}{\textbf{Trabajo de Fin de Máster}}	\\ 
				\multicolumn{2}{|c|}{}	\\ \hline
				\multicolumn{2}{|l|}{\textbf{ALUMNO}: Aitor García Blanco}	\\ \hline
				\multicolumn{2}{|l|}{\textbf{TUTOR}: Laura Fernández Robles}	\\ \hline
				\multicolumn{2}{|p{16cm}|}{\textbf{TÍTULO}: Detección de células redondas sobre un conjunto de imágenes médicas usando aprendizaje profundo} \\ \hline
				\multicolumn{2}{|p{16cm}|}{\textbf{TITLE}: Round Cell Detection in Medical Images Using Deep Learning} \\ \hline
				\multicolumn{2}{|l|}{\textbf{CONVOCATORIA}: Septiembre, 2025}		\\ \hline
				\multicolumn{2}{|l|}{\textbf{RESUMEN}:} \\ 
				\multicolumn{2}{|l|}{}\\
				\multicolumn{2}{|p{16cm}|}{
                } \\ 
				\multicolumn{2}{|l|}{}\\
				\hline
				\multicolumn{2}{|l|}{\textbf{ABSTRACT}:} \\ 
				\multicolumn{2}{|l|}{}\\
				\multicolumn{2}{|p{16cm}|}{
                } \\ 
				\multicolumn{2}{|l|}{}\\
				\hline
				\multicolumn{2}{|p{16cm}|}{\textbf{Palabras clave}: detección de objetos (\textit{object detection}), células redondas (\textit{round cells}), 
				análisis de semen (\textit{semen analysis}), inteligencia artificial (\textit{artificial intelligence}), aprendizaje profundo (\textit{deep learning}), modelos YOLO, reproducción asistida (\textit{assisted reproductive technology})}	\\ \hline
				\textbf{Firma del alumno}:\hspace{40mm} & \textbf{VºBº Tutor:}\\ 
				 & \\
				 & \\
				 & \\
				 & \\
				\hline
			\end{tabular}
		}
	\end{table*}
