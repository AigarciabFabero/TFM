...
\hline
\textbf{Descripción} & \multicolumn{3}{m{15cm}|}{Desarrollar y entrenar un modelo ensemble y un modelo personalizado. Evaluar cuantitativamente todos los modelos (base, \textit{ensemble} y personalizado) para seleccionar el mejor.} \\
\hline
\textbf{Criterios de aceptación} & \multicolumn{3}{m{15cm}|}{El modelo ensemble y el modelo personalizado deben estar implementados y entrenados. Se debe realizar una evaluación comparativa con métricas (mAP, precisión, \textit{recall}) y documentar los resultados para justificar la selección del modelo final.} \\
\hline
\end{tabular}%
}
...
\hline
\textbf{Descripción} & \multicolumn{3}{m{15cm}|}{Diseñar, implementar y probar la herramienta web con Streamlit, integrando los diferentes modelos de YOLO para permitir la detección de células en tiempo real.} \\
\hline
...
Realización continuada de pruebas y control de versiones \\
\hline
...
\begin{table}[H]
  ...
  \begin{tabular}{lccccc}
      ...
      ensemble   & 0.815 & \textbf{0.895} & 0.864 & 0.697 & 75.073 \\
      \hline
  \end{tabular}
  }
...
\arrayrulecolor{black}\hline
custom 0.5        & \cellcolor{myred!30}\ding{55} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{myred!30}\ding{55} & \cellcolor{myred!30}\ding{55} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{myred!30}\ding{55} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{myred!30}\ding{55} \\
\arrayrulecolor{mygray}\hline
custom 0.7        & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{myred!30}\ding{55} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} \\
\arrayrulecolor{black}\hline
ensemble       & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{myred!30}\ding{55} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} & \cellcolor{mygreen!30}\ding{51} \\
\arrayrulecolor{black}\hline
\end{tabular}
}
...
\begin{itemize}
  \item \textbf{Subgetividad y ambigüedad en las anotaciones:} La principal limitación del proyecto radica en la propia naturaleza del problema. Como se documenta en la literatura, 
  ...
  \item que una clasificación más detallada podría proporcionar.
  \item \textbf{Alcance y representatividad del dataset:} Auqnue el conjunto de datos proporcionado por Microptic S.L. \cite{microptic} es suficiente para demostrar la viabilidad de la prueba de concepto, 
  en número de datos que lo conformas ne bastante limitado si atendemos a los grandes volúmenes de datos que suelen abordar los modelos de aprendizaje profundo. Además, todos los datos provienen de la misma empresa, limitando así, la capacidad de generalizar de los modelos
  y la externalización del \textit{software} a otros laboratorios.
  \item \textbf{Recursos computacionales:} Si bien durante el proceso de entrenamiento se emplea \textit{hardware} bastante limitado, el uso de un servidor, pese a las implicaciones económicas que conlleva, puede ar solución a estas limitaciones.
...
Los resultados de este proyecto abren nuevas y prometedoras vias de investigación. Para continuar avanzando en la detección automatizada de células redondas se proponen las liguientes lineas de trabajo:

\begin{itemize}
  \item \textbf{Exploración de otras arquitecturas:} Esta prueba de concepto se ha enfocado en detectores de una única estapa (YOLO), optimizados para ofrecer un equilibrio entre precisiñon y velocidad. Sin embargo, 
  una alternativa a estos modelos pueden ser los de dos etapas (como \textit{Fast-RCNN}), el cual presenta generalmente mayor \textit{precision} y tiempo de inferencia. Una alternativa a esta linea de investigación es usar detectores basados en Transformers
  (como D-FINE). Este enfoque promete una localización muy precisa incluso en escenas complejas y podría superar algunas limitaciones de los modelos puramente convolucionales.
  ...
  dando lugar a falsos positivos por parte de las diferentes arquitectiras, incluida YOLOv12s.
  ...
  \item \textbf{Evolución y validación de la herramienta web:} La evolución de la herramienta vez debe pasar por la inclusión de nuevos modelos como \textit{Fast-RCNN}. El siguiente paso lógico es desplegar la herramienta web en un entorno de laboratorio real 
